...
这份路线图旨在通过实战练习，帮助安全工程师掌握云原生环境下的攻防技术。从代码审查到运行时监控，覆盖了完整的安全生命周期。

这份路线图已经拆分为不同的模块。请进入对应的目录查看任务详情，并在该目录下保存你的练习文件（如 YAML, 脚本, 截图等）。

> **🤝 协作方式**: 建议在每个阶段目录下创建一个 `solutions.md` 或存放相应的练习代码。你可以对照“通关标准”来验证自己的学习成果。
...
=======
*   **重点**: **CKS (必考)**, AWS Security, 面试与求职建议
*   **分析**: 云原生安全职业演化指南
>>>>>>> 364633f (update)
